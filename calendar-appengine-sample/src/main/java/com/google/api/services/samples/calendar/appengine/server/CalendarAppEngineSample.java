...
public class CalendarAppEngineSample extends AbstractFlowUserServlet {
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    writer.println(
        "<script type=\"text/javascript\" language=\"javascript\" " + "src=\"" + GWT_MODULE_NAME
            + "/" + GWT_MODULE_NAME + ".nocache.js\"></script>");
  ...
  protected PersistenceManagerFactory getPersistenceManagerFactory() {
    return Utils.getPersistenceManagerFactory();
  }
  ...
  protected ThreeLeggedFlow newFlow(String userId) throws IOException {
    return new GoogleOAuth2ThreeLeggedFlow(userId, OAuth2ClientCredentials.CLIENT_ID,
        OAuth2ClientCredentials.CLIENT_SECRET, CalendarUrl.ROOT_URL, Utils.getCallbackUrl());
  }
  ...
  protected HttpTransport newHttpTransportInstance() {
    return Utils.getTransport();
  }

  @Override
  protected JsonFactory newJsonFactoryInstance() {
    return Utils.getJsonFactory();
  }
}
