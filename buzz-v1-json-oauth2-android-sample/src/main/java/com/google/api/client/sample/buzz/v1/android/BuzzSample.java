...
public class BuzzSample extends Activity {
  ...
  final Buzz buzz = new Buzz("Google-BuzzSample/1.0", transport, new JacksonFactory());
  ...
  class LoadActivities extends AsyncTask<Void, Void, ActivityFeed> {
    ...
    protected ActivityFeed doInBackground(Void... params) {
      ...
      try {
        com.google.api.services.buzz.v1.Buzz.Activities.List request =
            buzz.activities.list("@me", "@consumption");
    ...
    protected void onPostExecute(ActivityFeed feed) {
      ...
      if (feed.items != null) {
        for (com.google.api.services.buzz.v1.model.Activity a : feed.items) {
